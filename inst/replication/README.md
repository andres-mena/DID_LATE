...
paper. The scripts are kept inside `inst/replication/` so they travel with the
R package while remaining separate from the user-facing API.
...
```bash
Rscript inst/replication/sim/01_dgp.R
Rscript inst/replication/sim/02_nuisance.R --method lasso
Rscript inst/replication/sim/03_estimators.R
Rscript inst/replication/sim/04_tables.R
Rscript inst/replication/sim/05_figures.R
Rscript inst/replication/sim/06_trimming_analysis.R
Rscript inst/replication/sim/07_ml_comparison_table.R
Rscript inst/replication/sim/trim_rule_summary.R
```

Empirical scripts:

```bash
Rscript inst/replication/empirical/01_data_duflo.R
Rscript inst/replication/empirical/02_nuisance.R --duflo
Rscript inst/replication/empirical/03_estimators.R --duflo
Rscript inst/replication/empirical/04_tables.R
Rscript inst/replication/empirical/06_balance_table.R
Rscript inst/replication/empirical/07_figures_appendix.R
Rscript inst/replication/empirical/08_figures_sensitivity.R
```
